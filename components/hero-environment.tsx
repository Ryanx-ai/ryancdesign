...
export function HeroEnvironment() {
  ...
  useEffect(() => {
    ...
    const stars: Star[] = Array.from({ length: 98 }, (_, index) => ({
      ...
      duration: 2100 + seeded(index, 6) * 8200,
    }));
    let width = 0, height = 0, frame = 0, rolePulse = 0; const visible = true;
    let sparkleIndex = -1, sparkleStart = 0, nextSparkle = 3200 + seeded(9, 8) * 3800;
    ...
    function draw(now: number) {
      ...
      if (animated && now > nextSparkle) {
        ...
        sparkleStart = now;
        nextSparkle = now + 3800 + seeded(Math.floor(now), 10) * 4600;
      }
      ...
      for (let index = 0; index < positions.length; index++) {
        ...
        const range = star.kind === "tiny" ? .07 : star.kind === "medium" ? .13 : .18;
        const baseOpacity = star.kind === "tiny" ? .28 : star.kind === "medium" ? .4 : .52;
