...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const liveProjectRoutes = getVisibleLiveProjects().flatMap((project) => project.status === "live" && project.caseStudyUrl ? [{ url: `${base}${project.caseStudyUrl}`, lastModified: new Date(), changeFrequency: "monthly" as const, priority: .9 }] : []);
